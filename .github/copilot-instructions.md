# Copilot Instructions for my-shooting-game
...
- **Build (web)**: Use `flutter build web --release --base-href /my-shooting-game/` so assets resolve correctly on GitHub Pages (path baked into workflow below). Copy `build/web/index.html` to `build/web/404.html` to support SPA routing.
...
- **Setup notes**: Project created via `flutter create --project-name my_shooting_game my-shooting-game`; repo name uses kebab-case, package uses snake_case (see rationale in [docs/setup-flutter-web-github-pages.md](../docs/setup-flutter-web-github-pages.md)). Keep `--base-href` aligned to repo name when forking/renaming.
